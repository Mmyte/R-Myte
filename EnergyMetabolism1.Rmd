...
load("/Users/Myte/Documents/DATA-filer/EnergyMet kohort/crc patcodes.RData")
...
kval = openxlsx::read.xlsx(xlsxFile="/Users/Myte/Documents/DATA-filer/EnergyMet kohort/Kvalitetsreg_160921_leverans.xlsx",
                           ...
                           skipEmptyRows=FALSE,
                           detectDates=T)
# Create unique ID variable and insert patcodes:
sum(patcodes$crc_id==inputcrc$crc_id)
inputcrc <- mutate(inputcrc,id=1:nrow(inputcrc),pat_code=patcodes$pat_code)
rm(patcodes)
crc <- inputcrc
...
kval1 <- dplyr::rename(kval1,crc_id=PERSNR, diaage.k=A0_alder, opererats=A2_oper_Värde, t=A3_t_Värde, n=A3_n_Värde, m=A3_m_Värde, diffgrad=A3_diffgrad_Värde, adenocarcinom=A3_adeno_Värde, dvikt=A2_vikt, dlangd=A2_langd, optyp1=A2_optyp1_Värde,optyp2=A2_optyp2_Värde) %>%
          mutate(
            diadat.k = ymd(A1_Diagnosdatum),
            dyear.k = as.numeric(substr(diadat.k,1,4)),
            opdat = ymd(A2_opdat),
            lokal = ifelse(A2_tumlok_Värde==1,A2_tumlage_Värde,10), 
            lokal2 = cut(lokal,c(0,1,9,11),labels=c("appendix","colon","rectum")))
summary(kval1)
...
dplyr::select(kval1,lokal2,opererats,optyp2,A1_ct1,A1_mr1,t,n,m) %>% apply(2,table,useNA="always")
...
tab(kval1$n, kval1$m)
# Skapa ny variabel:
kval1["stage"] <- ifelse((kval1$t==1|rna$T==2)&rna$N==0&rna$M==0,1,
                      ifelse((rna$T==3|rna$T==4|rna$T==5)&rna$N==0&rna$M==0,2,
                      ifelse(rna$N!=0&rna$M==0,3,
                      ifelse(rna$M!=0,4,9999))))
# Grade: endast från 2013
table(kval1$diffgrad, useNA="always")
# mer än en cancer? 1066 unika fall, 34 st har mer än 1 post
id <- as.numeric(names(table(kval1$crc_id)[table(kval1$crc_id)>1]))
filter(kval1,crc_id %in% id) %>% dplyr::select(crc_id, diadat.k, dyear.k, diaage.k, lokal, lokal2 ,opererats, opdat, t, n, m, diffgrad, dvikt, dlangd) %>%  View()
filter(kval1,crc_id %in% id) %>% dplyr::select(crc_id) %>% table()
...
kval2 <- dplyr::select(kval1, crc_id, diadat.k, dyear.k, diaage.k, lokal, lokal2 ,opererats, opdat, t, n, m, diffgrad, dvikt, dlangd) %>% 
        distinct(crc_id, .keep_all=T)
summary(kval2)
sum(crc$crc_id %in% kval2$crc_id)
sum(crc$fall==1&(crc$crc_id %in% kval2$crc_id))
# Merge kval-reg variables to large crc file
...
bg <- read.csv("~/Documents/DATA-filer/MSI BRAF.csv", sep=";", dec=",",na.strings=c(""," ","NA"))
bg <- filter(bg,fall==1) %>% dplyr::select(idnr,pat_code,Filter_folate, pyear, Age, delproj_num, PAD, dia_age2, 
                             ...
                             dplyr::rename(pyear.bg=pyear, age.bg=Age, diadat.bg=diadat2, lokal.bg=Lokal, lokal3.bg=Lokal_3,
...
#
#vilka crc-fall i bg finns i NSHDS-enkät? 636 stycken! 587 unika fall:
...
# vilka i NSHDS finns i crums? Några är med men ej prospektiva fall 
...
crums <- dplyr::select(crums, crc_id, pad, alder, opdatum, dodsdat, fudat, lokal:diffgrad, t.typ, t.front,
                       postkomp.filter, overlevnad.ve.121014, corrstat.ve.121014,
                       kras2grp, braffinal, mmr.2gr.man, k.neg.hi.lonew, cd3.til) %>%
                dplyr::rename(diaage.c=alder,opdatum.c=opdatum,dodsdat.c=dodsdat,
...
tab(crc$study)
levels(crc$study) <- c("not crc","not validated","crums","folat","folat.crums", "kvalreg","kvalreg.crums","kvalreg.folat","kvalreg.folat.crums")
crc$study <- fct_relevel(crc$study,c("kvalreg.folat.crums","kvalreg.folat","kvalreg.crums","folat.crums", 
                                             "kvalreg","folat","crums","not validated","not crc"))
tab(crc$study,crc$measurement)
crc["validated"] <- crc$study; levels(crc$validated) <- c(rep("validated",7),"not validated","not crc")
tab(crc$validated,crc$measurement)

# Merga allt: 1542 unika fall, 2733 measurements ---------------
# kval reg
...
# Jämför data:
crc1 %>% dplyr::select(crc_id, validated, study, kvalreg, folatbg, crums, DIADAT, diadat.k, diadat.bg, 
                       kvaldiff, bgdiff, 
                       diaage, diaage.k, diaage.bg, diaage.c,
                       lokal2, lokal3.bg, lokal3.c, lokal, lokal.bg, lokal.c,  
                       t, n, m, t.bg, n.bg, m.bg, t.c, n.c, m.c , stage.bg, stage.c, Filter_folate) %>% View()
# Jämför data för measurement == 1
crc1 %>% filter(measurement==1) %>% 
  dplyr::select(crc_id, validated, study, kvalreg, folatbg, crums, DIADAT, diadat.k, diadat.bg, 
                       kvaldiff, bgdiff, 
                       diaage, diaage.k, diaage.bg, diaage.c,
                       lokal2, lokal3.bg, lokal3.c, lokal, lokal.bg, lokal.c,  
                       t, n, m, t.bg, n.bg, m.bg, t.c, n.c, m.c , stage.bg, stage.c, Filter_folate) %>% View()
...
# 170 ej validerade + 135 av kvalreg validerade har annat datum än från cancerreg 
