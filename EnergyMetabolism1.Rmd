...
# set this individual to Filter_folate == 2
#bg[bg$idnr==4385,"fall"] <- 1
bg[bg$idnr==4385,"Filter_folate"] <- 2
bg <- filter(bg,fall==1) %>% dplyr::select(
                                           idnr,pat_code,Filter_folate, pyear, Age, delproj_num, PAD, dia_age2, 
...
crc2 <- crc1 %>% filter(measurement==1) %>% 
         mutate_at(c(193:194, 227:230), funs(as.integer)) %>%
...
crc <- dplyr::select(crc, crc_id, crc, fall, measurement:dbt, uppfdat:g6_comb)
