...
phen["mets"] <- scale(phen$bmi)+scale(phen$midbp)+scale(phen$blods0)+scale(phen$skol)+scale(phen$stg)
phen$mets <- as.numeric(phen$mets)
...
natcom <- natcom[,c("var","Age","Sex","Length","Weight","BMI","SBP","DBP","Smoking","Snus")]
...
colnames(modd) <-colnames(mod) <- c("Baseline","Case","Age","Sex","Length","Weight","BMI","Physical activity","Colesterol","Triglycerides","P-glucose","Glucose tolerance","SBP","DBP","Smoking","Snus","Medications")
...
# Matrix to store estimate in:
mod1 <- coef1 <- mod2 <- coef2 <- matrix(nrow=ncol(dat),ncol=length(cov))
colnames(mod1) <- colnames(coef1) <- colnames(mod2) <- colnames(coef2) <- c("Age","Sex","Length","Weight","Physical activity","Colesterol","Triglycerides","P-glucose","Glucose tolerance","SBP","DBP","Smoking","Snus","Medications")
rownames(mod1) <- rownames(coef1) <- rownames(mod2) <- rownames(coef2) <- var1 <- names(dat)
#var1 <- sort(var1)
rdata1 <- dat
j=0
...
for(i in var1){
  ...
  for(l in 1:p){
    ...
    a <-   anova(g,test="Chisq")
    mod1[j,l] <- a$`Pr(>Chi)`[3]
    mod2[j,l] <- a$`Pr(>Chi)`[19] #19
    coef2[j,l] <- coef(g)[21] #21
    #För att få rätt coefficients:
    formula1 <- as.formula(paste("moddat[,i]","~","+casecontrol+",cov[l],"+provnr+",paste(cov[-l],collapse="+"),sep=""))
    #formula1 <- as.formula(paste("moddat[,i]","~","+casecontrol+",cov[l],"*provnr+","gender+age",sep=""))
    g2 <- glm(formula1,data=moddat) #fit model remove NAs  
    coef1[j,l] <- coef(g2)[3]
  }
  # TRANSFORM data for correlation analysis
  g <- glm(moddat[,i]~.,data=moddat[,-1]) #fit model remove NAs
  ...
  rdata1[,j] <- rntransform(dat1[,i],data=dat1[,covt],family="gaussian") 
}

# Estimates: main effect
modf.bh <- apply(mod1,2,p.adjust,method="bonferroni")
modm <- data.frame(var=rownames(modf.bh),ifelse(modf.bh<=0.05,1,0)) 
...
nam <- c("Age","Sex","Length","Weight","Physical.activity","Colesterol","Triglycerides","P.glucose","Glucose.tolerance","SBP","DBP","Smoking","Snus","Medications")
str(dat.lme[,cov])
k=0
for(i in cov){
  k=k+1
...
#Age that are the same?



# MODEL 2 - Exempel på "significant" interaction med provnr:

# Glucose test and CD244, beror på en OBS skitlångt borta
gdata <- dat.lme[,c("blods2","provnr","FGF_5")]
gdata$provnr <- as.factor(gdata$provnr)
gdata2 <- melt(gdata,id=c("blods2","provnr"))

ggplot(gdata2,aes(y=value,x=blods2,fill=provnr))+
  geom_smooth(method="lm",color="black")+
  geom_point(aes(color=provnr), size=0.5)+
  facet_wrap(~variable,scales="free")+theme_bw()

# Sex and ERBB3
gdata <- dat.lme[,c("gender","provnr","ERBB3")]
gdata$gender <- as.factor(gdata$gender)
gdata$provnr <- as.factor(gdata$provnr)
gdata2 <- melt(gdata,id=c("gender","provnr"))

ggplot(gdata2,aes(y=value,x=provnr,fill=gender))+
  geom_boxplot(outlier.shape=NA,alpha=0.2) + #avoid plotting outliers twice
  geom_point(aes(color=gender),position = position_jitterdodge(), size=0.5)+
  facet_wrap(~variable,scales="free")+theme_bw()

# Medications
s1 <- signl[,c(1,16)]
s1 <- s1[s1[,2]>0,]
nn2 <- as.character(s1[,1])

gdata <- dat.lme[,c("med","provnr",nn2)]
gdata$med <- as.factor(gdata$med)
gdata$provnr <- as.factor(gdata$provnr)
gdata2 <- melt(gdata,id=c("med","provnr"))

ggplot(gdata2,aes(y=value,x=provnr,fill=med))+
  geom_boxplot(outlier.shape=NA,alpha=0.2) + #avoid plotting outliers twice
  geom_point(aes(color=med),position = position_jitterdodge(), size=0.5)+
  facet_wrap(~variable,scales="free")+theme_bw()
...
gdata <- dat.lme[,c("g6","provnr",nn2)]
#gdata$g6 <- as.factor(gdata$g6)
gdata$provnr <- as.factor(gdata$provnr)
gdata2 <- melt(gdata,id=c("g6","provnr"))

ggplot(gdata2,aes(y=value,x=g6,fill=provnr))+
  geom_smooth(method="lm",color="black")+
  geom_point(aes(color=provnr),position = position_jitterdodge(), size=0.5)+
  facet_wrap(~variable,scales="free")+theme_bw()

# P-glucose and CD244
gdata <- dat.lme[,c("blods0","provnr","CD244")]
gdata$provnr <- as.factor(gdata$provnr)
gdata2 <- melt(gdata,id=c("blods0","provnr"))

ggplot(gdata2,aes(y=value,x=blods0,fill=provnr))+
