...
res.proteins %>% dplyr::select(one_of(as.character(outliers$variable))) %>% 
  ...
  melt(id=c("index","id")) %>% 
  ggplot(aes(x=index,y=value)) + geom_point() + 
  #geom_text(data=gdat[is.na(gdat$value)==F&abs(gdat$value)>3,],aes(x=index, y=value, label=id),size=2,vjust="inward",hjust="inward") + 
...
Plot R2
```{r r squared plot}
r2 <- mutate(r2,icc=icc, icc4=cut(icc, breaks=c(0,0.25,0.5,0.75,1),labels=c("<25%","25-50%","50-75%",">75%")), r2mchange=r2m.mets-r2m,
...
volc <- data.frame(var=gsub("_","-",names(icc.mets)),icc=icc.mets,coef=coef.mets1[,1] ,p=mod.mets1[,1],
                   r2m.mets3=r2$r2m.mets3, r2mchange=r2$r2mchange) %>% 
  mutate(plog=-log10(p), icc4=cut(icc, breaks=c(0,0.25,0.5,0.75,1)))

ggplot(volc, aes(y=plog, x=coef, label=var, color=r2m.mets3)) + 
...
#Variables with potential outliers found from inspecting histograms::
# alla med >3 i pearson residual:
gdat <- res.proteins.d %>%mutate(index=seq(1,nrow(res.proteins.d)), id=as.character(dat.rep$id)) %>% melt(id=c("index","id")) 
outliers.d <- gdat[is.na(gdat$value)==F&abs(gdat$value)>3,]
tab(drop.levels(outliers.d$variable))

# plotta de med över 3 i pearson residual visas
res.proteins.d %>% dplyr::select(one_of(as.character(outliers$variable))) %>% 
  mutate(index=seq(1,nrow(res.proteins)), id=as.character(dat.lme$id)) %>% 
  melt(id=c("index","id")) %>% 
  ggplot(aes(x=index,y=value)) + geom_point() + 
  #geom_text(data=gdat[is.na(gdat$value)==F&abs(gdat$value)>3,],aes(x=index, y=value, label=id),size=2,vjust="inward",hjust="inward") + 
...
volc <- data.frame(var=gsub("_","-",rownames(mod.mets1)), r2=r2.d$r2.mets, sign=r2.d$sign, r2mchange=r2.d$r2mchange,
                   r2m.mets3 =r2.d$r2m.mets3, coef=coef.mets1.d[,1], p=mod.mets1.d[,1]) %>% 
  mutate(plog =-log10(p))
