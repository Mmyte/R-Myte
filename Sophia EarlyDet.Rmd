...
```{r 1 heatmap all IP, fig.width=12, fig.height=8, message=FALSE, warnings=FALSE}
...
```{r 2 standardized cluster analysis ,echo=F, fig.pos = "H",fig.width=8, fig.height=8}
...
p <- length(cov)
covm <- c("casecontrol","age","gender","phys","smoke","snus","med","bmi","bmi3","metsz","mets2","ht2","htm") 
pm <- length(covm)
# Matrix to store estimate in:
...
# rownames
rownames(mod1) <- rownames(coef1) <- rownames(mod2) <- rownames(mod.mets1) <- rownames(coef.mets1)  <- rownames(mod.mets2) <- rownames(sd1) <- rownames(sd.mets1) <- var1 
...
for(i in var1){
 ...
 # MODELs MetS
    ...
    g1 <- lme(var~bmi3+casecontrol+age+gender+phys+smoke+snus+med,
              random = ~1|person, na.action=na.omit,method="ML",data=moddat)
    g2 <- lme(var~bmi+casecontrol+age+gender+phys+smoke+snus+med,
              random = ~1|person, na.action=na.omit,method="ML",data=moddat)
    g3 <- lme(var~mets2+casecontrol+age+gender+phys+smoke+snus+med,
              random = ~1|person, na.action=na.omit,method="ML",data=moddat)
    g4 <- lme(var~metsz+casecontrol+age+gender+phys+smoke+snus+med,
              random = ~1|person, na.action=na.omit,method="ML",data=moddat)
    g5 <- lme(var~ht2+casecontrol+age+gender+phys+smoke+snus+med+bmi,
              random = ~1|person, na.action=na.omit,method="ML",data=moddat)
    g6 <- lme(var~htm+casecontrol+age+gender+phys+smoke+snus+med+bmi,
              random = ~1|person, na.action=na.omit,method="ML",data=moddat)
    # P-values
    ...
    # Coefficients
    coef.mets1[j,1:3] <- summary(g1)$tTable[2:4,1]
    coef.mets1[j,4] <- summary(g2)$tTable[2,1]
    coef.mets1[j,5] <- summary(g3)$tTable[2,1]
    coef.mets1[j,6] <- summary(g4)$tTable[2,1]
    coef.mets1[j,7] <- summary(g5)$tTable[2,1]
    coef.mets1[j,8] <- summary(g6)$tTable[2,1]
    # sd
    sd.mets1[j,1:3] <- summary(g1)$tTable[2:4,2]
    sd.mets1[j,4] <- summary(g2)$tTable[2,2]
    sd.mets1[j,5] <- summary(g3)$tTable[2,2]
    sd.mets1[j,6] <- summary(g4)$tTable[2,2]
    sd.mets1[j,7] <- summary(g5)$tTable[2,2]
    sd.mets1[j,8] <- summary(g6)$tTable[2,2]
...
mod.bh0 <- apply(mod1,2,p.adjust,method="bonferroni")
mod.bh1 <- apply(mod.mets1,2,p.adjust,method="bonferroni")
sop <- data.frame(padj=mod.bh0[,2],age=coef1[,2],agesd=sd1[,2],
                  ...
                  padj=mod.bh0[,7],chol=coef1[,7],cholsd=sd1[,7],
                  padj=mod.bh0[,8],trig=coef1[,8],cholsd=sd1[,8],
                  padj=mod.bh0[11],sbt=coef1[,11],sbtsd=sd1[,11],
                  padj=mod.bh0[,12],dbt=coef1[,12],dbtsd=sd1[,12],
                  padj=mod.bh1[,5],HT=coef.mets1[,7],HTsd=sd.mets1[,7],
                  padj=mod.bh1[,6],HTmed=coef.mets1[,8],HTmedsd=sd.mets1[,8],
                  padj=mod.bh1[,1],BMI20=coef.mets1[,1],BMI20sd=sd.mets1[,1],BMI2530=coef.mets1[,2],BMI2530sd=sd.mets1[,2],BMI30=coef.mets1[,3],BMI30sd=sd.mets1[,3],
                  padj=mod.bh1[,2],BMIcont=coef.mets1[,4],BMIcontsd=sd.mets1[,4],
...
#write.xlsx(rbind(p[p[,2]<0.05,],pd[pd[,2]<0.05,]), file ="hej.xlsx",sheetName = "TestSheet", row.names = T)
#write.xlsx(cbind(p[nn,],pd[nn,]), file ="hej.xlsx",sheetName = "TestSheet", row.names = T)
write.xlsx(sop, file ="sophia4.xlsx",sheetName = "TestSheet", row.names = T)
...
```{r,echo=F}
#Data fix --------------------
rdata1 <-  filter(dat.lme, provnr==1|2) %>% dplyr::select(one_of(var1),-TRAIL) 
...
#Data fix --------------------
# Significant BMI and Mets
m1 <- apply(mod.mets1[,-c(6)],2,p.adjust,method="bonferroni")
m2 <- apply(mod1[,c("Colesterol","Triglycerides","P.glucose")],2,p.adjust,method="bonferroni")
mm <- cbind(m1,m2)
...
rdata1 <-  filter(dat.lme, provnr==1|2) %>% dplyr::select(one_of(as.character(mets.sign$var)),-TRAIL)
names(rdata1) <- gsub('_', '-', names(rdata1))
...
lf1 <- filter(mets.sign,var!="TRAIL") %>% dplyr::select(-sum.sign) %>% melt(id="var") %>% 
        mutate(variable=factor(variable, labels=c("BMI (groups)","BMI (continuous)","MetS (groups)", "MetS (score)", "Hypertension","Cholesterol","Triglycerides","P-glucose"))) #%>% filter(variable!="HT",variable!="HT medication")
lf1 <- lf1[lf1$value>0,]
head(lf1);nrow(lf1)
levels(lf1$var) <- gsub('_', '-', levels(lf1$var))
lf1$var <- as.character(lf1$var)
#
#Add empty labels for Colesterol-DBP
lf1 <- rbind(lf1,c(" ",levels(lf1$variable)[6],0.5))
lf1 <- rbind(lf1,c(" ",levels(lf1$variable)[8],0.5))
#
