...
# MetS variables
# Z-transformed variables per gender and sample: doesn not change when not doing it on sample
phen<- transform(phen, 
          bmiz=ave(bmi, gender, provnr,  FUN=scale),
          stgz=ave(stg, gender, provnr,  FUN=scale),
          midbpz=ave(midbp, provnr, gender,  FUN=scale),
          blods0z=ave(blods0, provnr, gender, FUN=scale),
...
#ICCs MetS:
var <- c("metsz","bmi","stgc","midbp","blods0","skol")
icc1 <- numeric(6)
names(icc1) <- var
j=0
for(i in var){
  j=j+1
  dat1["var"] <- dat1[,i]
    g <- lme(var~casecontrol+age+gender+smoke, random =list(~1|person),
            na.action=na.exclude, method="ML",
            data=dat1)
  #icc empty
    varests <- as.numeric(VarCorr(g)[c(1,2)]) 
    icc1[j] <- varests[1]/sum(varests)
}
icc1 <- round(icc1,2)
...
names(icc1) <- levels(d3$variable)
levnames <- paste0(names(icc1) ,", ", sprintf("ICC: %.2f", icc1[names(icc1)]))
levels(d3$variable) <- levnames

# plot   
ggplot(d3, aes(y=value,x=age,group=person,color=Change)) + 
  geom_point(aes(shape=Sex),size=2) + geom_line(alpha=0.7) + 
   #geom_text(data=icc1,aes(label=icc))+
...
#Plot ICC: doesnt get better if scale
lev <- names(sort(icc.mets[as.character(mets.sign$var)],decreasing=T))
levnames <- paste0(lev,", ", sprintf("ICC: %.2f",icc.mets[lev]))
dplyr::select(dat.lme, person, provnr ,one_of(as.character(mets.sign$var))) %>%  
  melt(id=c("person","provnr")) %>%
  mutate(variable=fct_relevel(variable, lev), variable=factor(variable, labels=levnames)) %>%
  ggplot(aes(y=value, x=as.factor(person),color=as.factor(provnr))) +
  geom_point(size=0.8) + 
  geom_line(aes(group=as.factor(person)),color="black",alpha=0.8) + 
  facet_wrap(~variable) + theme(axis.text.x=element_blank(),axis.ticks.x=element_blank())
...
# Load list of batch data
batches <- read.csv("~/Documents/DATA-filer/Sophiaearlydetection/methylation/CpG batches.csv", sep=";", stringsAsFactors=FALSE)
batches <- dplyr::rename(batches, id=Sample,Case_set=Case.set,Chip=Chip..,Analysis=Analysis..)
...
#merge with olink/phen data frame and baatches:
...
sum(beta1$id %in% batches$id)
sum(batches$id %in% phen$id)

dat1cpg <- merge(batches, beta1, by="id")
dat1cpg <- merge(dat1cpg, phen, by="id")
...
#Associations to MetS
dplyr::select(dat1cpg, metsz, bmiz,stgz,midbpz, blods0z, skolz ,CD8T, CD4T, NK, Bcell, Gran, Mono) %>% 
  dplyr::rename(MetS=metsz, BMI=bmiz, Triglycerides=stgz, midBP=midbpz, P_glucose=blods0z, Cholesterol=skolz) %>%
  pairs(., diag.panel = panel.hist, upper.panel = panel.smooth,lower.panel = panel.cor,pch=19,cex=0.2,cex.labels=1, gap=0.2,col.smooth="black",col="gray")

#ICCs cell types:
var <- c("CD8T", "CD4T", "NK", "Bcell", "Gran", "Mono")
icc2 <- numeric(6)
names(icc2) <- var
j=0
for(i in var){
  j=j+1
  dat1cpg["var"] <- dat1cpg[,i]
    g <- lme(var~casecontrol+age+gender+smoke, random =list(~1|person),
            na.action=na.exclude, method="ML",
            data=dat1cpg)
  #icc empty
    varests <- as.numeric(VarCorr(g)[c(1,2)]) 
    icc2[j] <- varests[1]/sum(varests)
}
round(icc2,2)

#Plot ICC:
lev <- names(sort(icc2,decreasing=T))
levnames <- paste0(lev,", ", sprintf("ICC: %.2f",icc2[lev]))
dplyr::select(dat1cpg, person, provnr,CD8T, CD4T, NK , Bcell, Gran, Mono) %>% 
  #filter(Mono<0.19) %>%
  melt(id=c("person","provnr")) %>% 
  mutate(variable=fct_relevel(variable, lev), variable=factor(variable, labels=levnames)) %>%
  ggplot(aes(y=value, x=as.factor(person),color=as.factor(provnr))) +
  geom_point(size=0.8) + 
  geom_line(aes(group=as.factor(person)),color="black",alpha=0.8) + 
  facet_wrap(~variable, scales="free") + theme(axis.text.x=element_blank(),axis.ticks.x=element_blank())

```


Heatmap of levels + batch effects check
Rows are variables, columns are observations. Observations are sorted according to case. Gray squares indicate missing observations.

```{r 1 heatmap CpGs, fig.width=12, fig.height=8, message=FALSE, warnings=FALSE}
load("~/Dropbox/Documents/JOBB ONKO/Sophia Early Detection/dat1cpg MED batch.RData")

cpg1 <- cpg1[!(cpg1 %in% c("cg04199023", "cg10588688", "cg24421822", "cg25862752"))] # remove missing cpgs from list
var <- cpg1

# Data fix  8*8 inch pic
dat11 <- dplyr::select(dat1cpg, provnr, casecontrol, person, Case_set, Chip, Analysis, BC_Plate, cg18478105:cg12623625)  %>% 
          mutate(case=factor(casecontrol,labels=c("control","case")), 
                 case.provnr=interaction(case,provnr), cpnum=as.numeric(case.provnr))

#Heatmap of factor-levels: (top)
gdat <- dplyr::select(dat11, person, Case_set, Chip, Analysis, BC_Plate) %>% 
  arrange(BC_Plate, Analysis, Chip, Case_set) %>% 
  mutate(order=1:nrow(dat11),BC_Plate=as.factor(BC_Plate), Case_set=as.factor(Case_set), Chip=as.factor(Chip), Analysis=as.factor(Analysis),
         variable1=rep("BC_Plate", nrow(dat11)), variable2=rep("Case_set", nrow(dat11)),
         variable3=rep("Chip", nrow(dat11)), variable4=rep("Analysis", nrow(dat11)))
# plots
g1 <- ggplot(gdat, aes(y=variable1, x=order)) + geom_tile(aes(fill = BC_Plate, color=BC_Plate)) +
    theme(line = element_blank(),
        title = element_blank(),
        legend.position="none",
        axis.title.x=element_blank(), axis.text.x=element_blank(),
        plot.margin=unit(c(0,0,-0.15,0), "cm"))
g2 <- ggplot(gdat, aes(y=variable2, x=order)) + geom_tile(aes(fill = Analysis, color=Analysis)) +
    theme(line = element_blank(),
        title = element_blank(),
        legend.position="none",
        axis.title.x=element_blank(), axis.text.x=element_blank(),
        plot.margin=unit(c(0,0,-0.15,0), "cm"))
g3 <- ggplot(gdat, aes(y=variable3, x=order)) + geom_tile(aes(fill = Chip, color=Chip)) +
    theme(line = element_blank(),
        title = element_blank(),
        legend.position="none",
        axis.title.x=element_blank(), axis.text.x=element_blank(),
        plot.margin=unit(c(0,0,-0.15,0), "cm"))
g4 <- ggplot(gdat, aes(y=variable4, x=order)) + geom_tile(aes(fill = Case_set, color=Case_set)) +
    theme(line = element_blank(),
        title = element_blank(),
        legend.position="none",
        axis.title.x=element_blank(), axis.text.x=element_blank(),
...
#Heatmap of CpG: (bottom)
dat11[,var] <- scale(dat11[,var])
gdat <- dplyr::select(dat11, person, Case_set, Chip, Analysis, BC_Plate, one_of(var)) %>% 
  arrange(BC_Plate, Analysis, Chip, Case_set) %>% 
  mutate(order=1:nrow(dat11)) %>% 
  melt(id=c("person","order","BC_Plate", "Analysis", "Chip", "Case_set"))
         
gBot <-  ggplot(gdat, aes(y=variable, x=order)) + 
  geom_tile(aes(fill = value, color=value)) +
  #scale_fill_gradient(low = "dodgerblue4", high = "gold", na.value="gray") +
  #scale_color_gradient(low = "dodgerblue4", high = "gold", na.value="gray") +
  scale_fill_gradient2(low = "dodgerblue4", mid="gray95", high = "firebrick4", na.value="gray") + 
  scale_color_gradient2(low = "dodgerblue4", mid="gray95", high = "firebrick4", na.value="gray") + 
  theme(legend.position="none",
        line = element_blank(),
         axis.text.x=element_blank(), axis.ticks.x=element_blank(),
         axis.text.y=element_blank(), axis.ticks.y=element_blank(),
          plot.margin=unit(c(0.05,0,0,0), "cm")) +
  ylab("CpG sites") + xlab("Participants")

# Merge plots together with cowplot
plot_grid(g1,g2,g3,g4, gBot, nrow = 5, ncol=1,  align = "v", rel_heights = c(1,1,1,1,16))


ggdraw() +
  draw_plot(plot.iris1, 0, .5, 1, .5) +
  draw_plot(plot.iris2, 0, 0, 1, .5) +
  draw_plot_label(c("A", "B"), c(0, 0), c(1, 0.5), size = 15)

```


The same but with PCAs
```{r 1 heatmap PCA, fig.width=12, fig.height=8, message=FALSE, warnings=FALSE}
load("~/Dropbox/Documents/JOBB ONKO/Sophia Early Detection/dat1cpg MED batch.RData")
...
# Data fix  8*8 inch pic
dat11 <- dplyr::select(dat1cpg, provnr, casecontrol, person, Case_set, Chip, Analysis, BC_Plate, 7:851200)  %>% 
          mutate(case=factor(casecontrol,labels=c("control","case")), 
                 case.provnr=interaction(case,provnr), cpnum=as.numeric(case.provnr))
...
#Plot loadings
melted <- data.frame(var=mets.cpg$var, Protein=mets.cpg$OLINK.Protein_name, pca$rotation[,1:5])
melted <- melt(melted, id=c("var","Protein"))
melted <- mutate(melted, index=rep(1:471, 5))
ggplot(data=melted) +
  geom_bar(aes(x=index, y=value, fill=Protein), stat="identity") +
  facet_wrap(~variable) +xlab("") + ylab("Loadings")
...
for(i in var1){
  ...
  # MODELs lifestyle
    ...
    r2cpg[j,9] <- r[2]
 # MODELs MetS
    g1 <- lme(var~CD8T+CD4T+NK+Bcell+Gran+Mono+metsz+casecontrol+age+gender+phys+smoke,
              random = ~1|person, na.action=na.exclude,method="ML",data=dat.lme)
    g2 <- lme(var~CD8T+CD4T+NK+Bcell+Gran+Mono+mets2+casecontrol+age+gender+phys+smoke,
              random = ~1|person, na.action=na.exclude,method="ML",data=dat.lme)
    g3 <- lme(var~CD8T+CD4T+NK+Bcell+Gran+Mono+bmiz+stgz+midbpz+blods0z+skolz+casecontrol+age+gender+phys+smoke,
              random = ~1|person, na.action=na.exclude,method="ML",data=dat.lme)
    g4 <- lme(var~CD8T+CD4T+NK+Bcell+Gran+Mono+bmi2+stg2+ht2+gluc2+skol2+casecontrol+age+gender+phys+smoke,
              random = ~1|person, na.action=na.exclude,method="ML",data=dat.lme)
    #ICC
    varests <- as.numeric(VarCorr(g1)[c(1,2)]) 
    ...
    # P-values
    mod.mets1cpg[j,1] <- anova(g1)$`p-value`[8]
    mod.mets1cpg[j,2] <- anova(g2)$`p-value`[8]
    mod.mets1cpg[j,3:7] <- anova(g3)$`p-value`[8:12]
    mod.mets1cpg[j,8:12] <- anova(g4)$`p-value`[8:12]

    # Coefficients
    coef.mets1cpg[j,1] <- summary(g1)$tTable[8,1]
    coef.mets1cpg[j,2] <- summary(g2)$tTable[8,1]
    coef.mets1cpg[j,3:7] <- summary(g3)$tTable[8:12,1]
    coef.mets1cpg[j,8:12] <- summary(g4)$tTable[8:12,1]

    # sd
    sd.mets1cpg[j,1] <- summary(g1)$tTable[8,2]
    sd.mets1cpg[j,2] <- summary(g2)$tTable[8,2]
    sd.mets1cpg[j,3:7] <- summary(g3)$tTable[8:12,2]
    sd.mets1cpg[j,8:12] <- summary(g4)$tTable[8:12,2]
    #Rsq mets
    g1 <- lme(var~CD8T+CD4T+NK+Bcell+Gran+Mono+metsz+casecontrol+age+gender+phys+smoke,
              random = ~1|person, na.action=na.omit,method="ML",data=dat.lme)
    r <-  r.squaredGLMM(g1)
...
```{r}
# Data fix
gdat <- data.frame(var=rownames(mod.mets1cpg), mod.mets1cpg, mod1cpg) %>% 
        dplyr::select(var,Age, Sex, Case,Physical.activity, Smoking,
                      MetS_score, BMI_score,Triglycerices_score,MidBP_score,P.glucose_score,Cholesterol_score)
                     # MetS_groups,BMI_groups,Triglycerices_groups,Hypertension,Glucose_groups,Cholesterol_groups)
...
gdat$variable <- fct_rev(gdat$variable)
  
# Plot continous tiles
...
ggplot(gdat,aes(x = var, y = variable)) + 
   ...
   scale_fill_manual(values=c("red","orange","white"), guide=guide_legend(title = "P-value"))+
...
for(i in var1){
  ...
  # MODELs lifestyle
  tryCatch({
    g <- lme(var~CD8T+CD4T+NK+Bcell+Gran+Mono+casecontrol+age+gender+langd+vikt+phys+skol+stg+blods0+blods2+sbt+dbt+
               smoke+snus+med,
            random =list(~1|person),
            na.action=na.exclude, method="ML",
            data=dat.lme)
    ...
    icc.lifestylecpg[j] <- varests[1]/sum(varests)
    #Rsq lifestyle 
    g <- lme(var~CD8T+CD4T+NK+Bcell+Gran+Mono+casecontrol+age+gender+langd+vikt+phys+skol+stg+blods0+blods2+
               sbt+dbt+smoke+snus+med,
            random =list(~1|person),
            na.action=na.omit, method="ML",
            data=dat.lme)
     r <-  r.squaredGLMM(g)
 ...
 # MODELs MetS
  tryCatch({
    ...
    mod.mets1cpg[j,3:7] <- anova(g3)$`p-value`[8:12]
    mod.mets1cpg[j,8:12] <- anova(g4)$`p-value`[8:12]
    # P-values wald, gives the same results as having WBC first then picking variables
    #mod.mets1cpg[j,1] <-  summary(g1)$tTable[8,5]
    #mod.mets1cpg[j,2] <- summary(g2)$tTable[8,5]
    #mod.mets1cpg[j,3:7] <- summary(g3)$tTable[8:12,5]
    #mod.mets1cpg[j,8:12] <- summary(g4)$tTable[8:12,5]
    # Coefficients
    coef.mets1cpg[j,1] <- summary(g1)$tTable[8,1]
    coef.mets1cpg[j,2] <- summary(g2)$tTable[8,1]
    coef.mets1cpg[j,3:7] <- summary(g3)$tTable[8:12,1]
...
#save(outlierscpg, file="outlierscpg.RData")
...
#Only one significant cpg
ggplot(dat1cpg, aes(x=person,y=cg02220196, color=as.factor(provnr))) + 
  geom_point() + 
  geom_line(aes(group=person), col="black",alpha=0.8) + ylim(c(0,0.1))
#
ggplot(dat1cpg, aes(x=skolz,y=cg02220196, color=as.factor(provnr))) + geom_point() + geom_smooth(method="lm") #+ ylim(c(0,1)) 
...
# adj p
mod.bh0 <- apply(mod1cpg,2,p.adjust,method="bonferroni")
mod.bh1 <- apply(mod.mets1cpg,2,p.adjust,method="bonferroni")
sop <- data.frame(r2cpg, icc=icc.metscpg, 
                  padj=mod.bh0[,2],age=coef1cpg[,2],agesd=sd1cpg[,2],
                  padj=mod.bh0[,3],sex=coef1cpg[,3],sexsd=sd1cpg[,3],
                  padj=mod.bh0[,4],height=coef1cpg[,4],heightsd=sd1cpg[,4],
                  padj=mod.bh0[,5],weight=coef1cpg[,5],weightsd=sd1cpg[,5],
                  padj=mod.bh0[,6],phys=coef1cpg[,6],physsd=sd1cpg[,6],

                  padj=mod.bh1[,1],MetS_score=coef.mets1cpg[,1],sd=sd.mets1cpg[,1],
                  padj=mod.bh1[,2],MetS_groups=coef.mets1cpg[,2],sd=sd.mets1cpg[,2],
                  padj=mod.bh1[,3],BMI_score=coef.mets1cpg[,3],sd=sd.mets1cpg[,3],
                  padj=mod.bh1[,4],Triglycerices_score=coef.mets1cpg[,4],sd=sd.mets1cpg[,4],
                  padj=mod.bh1[,5],MidBP_score=coef.mets1cpg[,5],sd=sd.mets1cpg[,5],
                  padj=mod.bh1[,6],P.glucose_score=coef.mets1cpg[,6],sd=sd.mets1cpg[,6],
                  padj=mod.bh1[,7],Cholesterol_score=coef.mets1cpg[,7],sd=sd.mets1cpg[,7],
                  
                  padj=mod.bh1[,8],BMI_groups=coef.mets1cpg[,8],sd=sd.mets1cpg[,8],
                  padj=mod.bh1[,9],Triglycerices_groups=coef.mets1cpg[,9],sd=sd.mets1cpg[,9],
                  padj=mod.bh1[,10],Hypertension=coef.mets1cpg[,10],sd=sd.mets1cpg[,10],
                  padj=mod.bh1[,11],P.glucose_groups=coef.mets1cpg[,11],sd=sd.mets1cpg[,11],
                  padj=mod.bh1[,12],Cholesterol_groups=coef.mets1cpg[,12],sd=sd.mets1cpg[,12])
#names(sop) <- c(names(r2),"icc",rep(c("padj","coef","sd"),17))             
sop <- sop[order(sop$var),]
sop2 <- merge(sop,mets.cpg, by="var",all.x=T)

# Export estimates to excel
#write.xlsx(rbind(p[p[,2]<0.05,],pd[pd[,2]<0.05,]), file ="hej.xlsx",sheetName = "TestSheet", row.names = T)
#write.xlsx(cbind(p[nn,],pd[nn,]), file ="hej.xlsx",sheetName = "TestSheet", row.names = T)
write.xlsx(sop2, file ="sophiaCPG2.xlsx",sheetName = "TestSheet", row.names = T)
...
Plot R2
```{r r squared plot}
r2cpg <- mutate(r2cpg, olink=mets.cpg$OLINK.Protein_name, 
             icc=icccpg, icc4=cut(icc, breaks=c(0,0.25,0.5,0.75,1),labels=c("<25%","25-50%","50-75%",">75%")),
             sign=ifelse(r2cpg$var %in% mets.signcpg$var,1,0), 
             r2mchange=r2m.mets-r2m,
             r2m.mets3=cut(r2m.mets,breaks=c(0,0.1,0.2,0.3),labels=c("<10%","10-20%","20-30%")),
             r2c.mets4=cut(r2c.mets,breaks=c(0,0.25,0.5,0.75,1),labels=c("<25%","25-50%","50-75%",">75%")),
             r2mchange3=cut(r2mchange,breaks=c(-0.1,0.05,0.1,0.15,0.20,0.3),labels=c("<5%","5-10%","10-15%","15-20%","20-30%")),
...
# MetS 
ggplot(r2cpg, aes(x=r2mchange, y=r2m.mets, color=icc4, label=var)) + geom_point(aes(size=r2mchange),alpha=0.8) + 
  geom_text(aes(size=r2mchange), nudge_x = 0.001,nudge_y = -0.005,vjust="inward",hjust="inward") + 
  geom_text(data=r2cpg[r2cpg$sign==1,], aes(x=r2mchange, y=r2m.mets, color=icc4, label=var, size=r2mchange), 
            nudge_x = 0.001,nudge_y = -0.005,vjust="inward",hjust="inward",col="black") + 
  scale_size(range = c(1,7), guide = FALSE) +
  scale_color_discrete(name = "ICC") +
  xlab("Change in variance explained\n by including MetS") + ylab("Variance explained by fixed factors")
...
# MetS components
ggplot(r2cpg, aes(x=r2mchange.comp, y=r2m.comp, color=icc4, label=var)) + geom_point(aes(size=r2mchange.comp),alpha=0.8) + 
  geom_text(aes(size=r2mchange.comp), nudge_x = 0.001,nudge_y = -0.005,vjust="inward",hjust="inward") + 
  geom_text(data=r2cpg[r2cpg$sign==1,], aes(x=r2mchange.comp, y=r2m.comp, color=icc4, label=var, size=r2mchange.comp), 
            nudge_x = 0.001,nudge_y = -0.005,vjust="inward",hjust="inward",col="black") + 
  scale_size(range = c(1,7), guide = FALSE) +
  scale_color_discrete(name = "ICC") +
  xlab("Change in variance explained\n by including MetS components") + ylab("Variance explained by fixed factors")
...
```{r volcano plot}
# p-threshold
pthreshold = -log10(0.05/length(icc.metscpg))

#MetS
volc <- data.frame(var=gsub("_","-",names(icc.metscpg)),icc=icccpg,coef=coef.mets1cpg[,1] ,p=mod.mets1cpg[,1],
                   r2m.mets3=r2cpg$r2m.mets3, r2mchange=r2cpg$r2mchange, 
                   r2mchange3=r2cpg$r2mchange3,r2mchange.comp3=r2cpg$r2mchange.comp3) %>% 
  mutate(plog=-log10(p), icc4=cut(icc, breaks=c(0,0.25,0.5,0.75,1)))

ggplot(volc, aes(y=plog, x=coef, label=var, color=r2mchange3)) + 
  geom_point(aes(size=abs(coef))) + 
  geom_text(aes(size=abs(coef)), nudge_x = 0.0002,vjust="inward",hjust="inward") + 
  geom_hline(yintercept=pthreshold,lty=2) +
  scale_size(range = c(1,6), guide = FALSE) + 
  #scale_color_discrete(name = "Variance explained\n by fixed factors") +
  scale_color_discrete(name = "Change in variance explained\n by including MetS") +
...
#BMI
volc <- data.frame(var=gsub("_","-",names(icc.metscpg)),icc=icccpg,coef=coef.mets1cpg[,3] ,p=mod.mets1cpg[,3],
                   r2m.mets3=r2cpg$r2m.mets3, r2mchange=r2cpg$r2mchange, 
                   r2mchange3=r2cpg$r2mchange3,r2mchange.comp3=r2cpg$r2mchange.comp3) %>% 
  mutate(plog=-log10(p), icc4=cut(icc, breaks=c(0,0.25,0.5,0.75,1)))
...
#Trig
volc <- data.frame(var=gsub("_","-",names(icc.metscpg)),icc=icccpg,coef=coef.mets1cpg[,10] ,p=mod.mets1cpg[,10],
                   r2m.mets3=r2cpg$r2m.mets3, r2mchange=r2cpg$r2mchange, 
                   r2mchange3=r2cpg$r2mchange3,r2mchange.comp3=r2cpg$r2mchange.comp3) %>% 
  mutate(plog=-log10(p), icc4=cut(icc, breaks=c(0,0.25,0.5,0.75,1)))
...
#P-glucose
volc <- data.frame(var=gsub("_","-",names(icc.metscpg)),icc=icccpg,coef=coef.mets1cpg[,6] ,p=mod.mets1cpg[,6],
                   r2m.mets3=r2cpg$r2m.mets3, r2mchange=r2cpg$r2mchange, 
                   r2mchange3=r2cpg$r2mchange3,r2mchange.comp3=r2cpg$r2mchange.comp3) %>% 
  mutate(plog=-log10(p), icc4=cut(icc, breaks=c(0,0.25,0.5,0.75,1)))
...
ggplot(volc, aes(y=plog, x=coef, label=var, color=r2mchange.comp3)) + 
  ...
  geom_text(aes(size=abs(coef)), nudge_x = 0.0002,vjust="inward",hjust="inward") + 
  geom_hline(yintercept=pthreshold,lty=2) +
  scale_size(range = c(4,6), guide = FALSE) + 
  #scale_color_discrete(name = "Variance explained\n by fixed factors") +
  scale_color_discrete(name = "Change in variance explained\n by including MetS components") +
  #theme_bw() +
  xlab("Coefficient P-glucose") + ylab("log10 P-value")
...
#Total cholesterol
volc <- data.frame(var=gsub("_","-",names(icc.metscpg)),icc=icccpg,coef=coef.mets1cpg[,7] ,p=mod.mets1cpg[,7],
                   r2m.mets3=r2cpg$r2m.mets3, r2mchange=r2cpg$r2mchange, 
                   r2mchange3=r2cpg$r2mchange3,r2mchange.comp3=r2cpg$r2mchange.comp3) %>% 
  mutate(plog=-log10(p), icc4=cut(icc, breaks=c(0,0.25,0.5,0.75,1)))
...
# SCore variables:
# Data frame: 
lf1 <-  dplyr::select(mets.signcpg, OLINK.Protein_name, MetS_score, BMI_score:Cholesterol_score, Hypertension) %>% 
  dplyr::rename(var=OLINK.Protein_name) %>%
  melt(id="var") %>% 
        mutate(variable=factor(variable, labels=c("MetS","BMI","Triglycerices", "Mid-BP", "P-glucose","Cholesterol","Hypertension")))
lf1 <- lf1[lf1$value>0,]
head(lf1);nrow(lf1)
...
#
#Add empty labels for MiD-BP, P-Glucose and Cholesterol
lf1 <- rbind(c(" ",levels(lf1$variable)[1],0.5),lf1)
lf1 <- rbind(c(" ",levels(lf1$variable)[2],0.5),lf1)
lf1 <- rbind(lf1, c(" ",levels(lf1$variable)[4],0.5))
...
#Correlations between CpG and protein markers 
...
# Partial correlations: 
cor1 <- cor(res.proteinscpg[,-472],method="spearman",use="pairwise.complete.obs")
#Grouping
fcol <- as.factor(mets.cpg$OLINK.Protein_name)
levels(fcol) <- scales::hue_pal()(length(levels(fcol)))
fcol <- as.character(fcol)
...
pairs(res.proteins[,as.character(mets.sign$var)], diag.panel = panel.hist, upper.panel = panel.smooth,lower.panel = panel.cor,pch=19,cex=0.2,cex.labels=1.2, gap=0.5,col.smooth="black",col="gray")
...
#Partial correlations
dim(res.proteins[res.proteins$id %in% res.proteinscpg$id,])
rdata1 <- merge(res.proteins,res.proteinscpg,by="id") %>% dplyr::select(one_of(as.character(mets.sign$var)), one_of(cpg1))
...
str(rdata1)
cor1 <- cor(rdata1, method="spearman", use="pairwise.complete.obs")
#Fix names of cpg sites
nn <- colnames(cor1)[13:length(colnames(cor1))]
sum(nn==mets.cpg$var)
colnames(cor1)[13:length(colnames(cor1))] <- paste0(mets.cpg$OLINK.Protein_name,".",mets.cpg$var)
rownames(cor1)[13:length(rownames(cor1))] <- paste0(mets.cpg$OLINK.Protein_name,".",mets.cpg$var)
# Melt
cor11 <- melt(cor1);nrow(cor11)
#Remove diagnonal in correlations matrix
cor11 <- cor11[cor11[,1]!=cor11[,2],];nrow(cor11)
...
cor11[,3] <- cor11[,3]^2
# Remove same correlations 
cor11 <- cor11[!duplicated(cor11[,3]),];nrow(cor11)
# Only those above 0.2
cor11[,4] <- ifelse(cor11[,3]>=0.10,1,0) 
tab(cor11[,4])
cor22 <- cor11[cor11[,4]==1,1:3]
hist(cor22[,3])
...
#Highlight cols
col <-  grid.col[match(cor22[,1],names(grid.col))]
#col[cor22[3] < 0.3] = "gray80"
col[cor22[3] < 0.20] = "gray90"
...
dd <- cor11[order(cor11[,3], decreasing = T),]

cor22["r2"] <- round(cor22[,3],2)
write.xlsx(cor22, file ="cor222.xlsx",sheetName = "TestSheet", row.names = T)
