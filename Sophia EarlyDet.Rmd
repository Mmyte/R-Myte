...
datx <- dplyr::select(datx,-HGF.y,-TGF_alpha,-VEGF_A.y) %>% rename(HGF=HGF.x,VEGF_A=VEGF_A.x)
datlod <- dplyr::select(datlod,-HGF.y,-TGF_alpha,-VEGF_A.y) %>% rename(HGF=HGF.x,VEGF_A=VEGF_A.x)
par(mfrow=c(1,1))
# MISSING OLINK-data
...
# colnames
colnames(mod1) <- colnames(coef1) <- colnames(mod2) <- colnames(coef2) <- colnames(sd1) <- c("Age","Sex","Length","Weight","Physical activity","Colesterol","Triglycerides","P-glucose","Glucose tolerance","SBP","DBP","Smoking","Snus","Medications")
...
lme1 <- lmer(ESM_1 ~ casecontrol+age+gender+langd+vikt+g6+skol+stg+blods0+blods2+sbt+dbt+smoke+snus+med+(1|person) , dat.lme, REML=T)
(s <- summary(lme1))
#Test
lme2 <- lmer(ESM_1 ~ casecontrol+age+gender+langd+vikt+g6+skol+stg+blods0+blods2+sbt+dbt+smoke+snus+med+(1|person) , dat.lme, REML=F)
# also works with lme objects
anova(lme1, lme2)

#With glm:
glm1 <- glm(ESM_1~g6+provnr+casecontrol+age+gender+langd+vikt+skol+stg+blods0+blods2+sbt+dbt+smoke+snus+med,dat.lme,family="gaussian")
summary(glm1)
anova(glm1,test="Chisq")

# Samma som med lmer (lme4)
lme3 <- lme(WFDC2 ~ casecontrol+age+gender+langd+vikt+g6+skol+stg+blods0+blods2+sbt+dbt+smoke+snus+med,
            random =list(~1|person, ~1|provnr),
            na.action=na.omit,
            data=dat.lme)
summary(lme3)
anova(lme3)

dplyr::select(dat.lme,WFDC2,person,provnr) %>% filter(person=="1_0"|person=="1_1") %>% head()
...
coef.mets1 <- coef.mets2 <- sd.mets1 <- matrix(nrow=ncol(dat),ncol=8) 
# colnames
colnames(mod1) <- colnames(coef1) <- colnames(mod2) <- colnames(coef2) <- colnames(sd1) <- c("Case","Age","Sex","Length","Weight","Physical activity","Colesterol","Triglycerides","P-glucose","Glucose tolerance","SBP","DBP","Smoking","Snus","Medications")
colnames(mod.mets1) <- colnames(mod.mets2)  <-  c("BMIgroups","BMIcont","MetSgroups","MetScont","HT","HTmed")
colnames(coef.mets1) <- colnames(coef.mets2) <- colnames(sd.mets1) <- c("BMI<20","BMI25-30","BMI>30","BMIcont","MetSgroups","MetScont","HT","HTmed")
# rownames
rownames(mod1) <- rownames(coef1) <- rownames(mod2) <- rownames(coef2) <- rownames(mod.mets1) <- rownames(coef.mets1)  <- rownames(mod.mets2) <- rownames(coef.mets2) <- rownames(sd1) <- rownames(sd.mets1) <- var1 
...
for(i in var1){
  ...
  # MODEL
    ...
    a <-   anova(g)
    mod1[j,] <- a$`p-value`[-1]
    varests <- as.numeric(VarCorr(g)[c(1,3)])
    icc[j] <- varests[1]/sum(varests)
  # MODEL BMI mets
    moddat <- na.omit(data.frame(var=dat.lme[,i],dat.lme[,c("person","provnr",covm)]))
    g <- lme(var~bmi3+casecontrol+age+gender+g6+smoke+snus+med,
            random = ~provnr|person,
            na.action=na.omit,
            data=moddat)
    a <-   anova(g)
    mod.mets1[j,1] <- a$`p-value`[2]
    moddat <- na.omit(data.frame(var=dat.lme[,i],dat.lme[,c("person","provnr",covm)]))
    g <- lme(var~mets2+casecontrol+age+gender+g6+smoke+snus+med,
            random =list(~1|person, ~1|provnr),
            na.action=na.omit,
            data=moddat)
    a <-   anova(g)
    mod.mets1[j,3] <- a$`p-value`[2]
}
