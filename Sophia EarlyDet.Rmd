...
mets.sign <- mutate(mets.bh2,sum.sign=rowSums(mets.bh2[,2:7])) %>% filter(sum.sign>0) 
...
###Plot R2
```{r r squared plot}
r2 <- mutate(r2,icc=icc, icc4=cut(icc, breaks=c(0,0.25,0.5,0.75,1)), r2mchange=r2m.mets-r2m,
...
# used plot
ggplot(r2, aes(x=r2mchange, y=r2c.mets, color=r2m.mets3, label=var)) + geom_point(aes(size=abs(r2mchange)),alpha=0.8) + 
  geom_text(aes(size=abs(r2mchange)), nudge_x = 0.001,nudge_y = -0.005,vjust="inward",hjust="inward") + 
  geom_text(data=r2[r2$sign==1,], aes(x=r2mchange, y=r2c.mets, color=r2m.mets3, label=var, size=abs(r2mchange)), 
            nudge_x = 0.001,nudge_y = -0.005,vjust="inward",hjust="inward",fontface="bold") + 
  scale_size(range = c(1,7), guide = FALSE) +
  scale_color_discrete(name = "Variance explained\n by fixed factors") +
  xlab("Change in variance explained\n by including MetS") + ylab("Variance explained by fixed + random factors")

#ggplot(r2, aes(x=r2m.mets, y=r2c.mets, color=icc4)) + 
#  geom_point(aes(size=abs(r2mchange))) + 
#  scale_size(range = c(1,8), guide = guide_legend(title="Change in variance explained\n by including MetS")) +
#  geom_text(data=r2[r2$sign==1,], aes(x=r2m.mets, y=r2c.mets, color=icc4, label=var), 
#            hjust = 0, nudge_x = 0.005,nudge_y=-0.01) + 
#  xlab("Variance explained by fixed factors") + ylab("Variance explained by fixed + random factors")
...
#MetS
volc <- data.frame(var=gsub("_","-",names(icc.mets)),icc=icc.mets,coef=coef.mets1[,1],p=mod.mets1[,1]) %>% 
  mutate(plog=-log10(p), icc4=cut(icc, breaks=c(0,0.25,0.5,0.75,1)))

ggplot(volc, aes(y=plog, x=coef, label=var, color=icc4)) + 
  geom_point(aes(size=abs(coef))) + 
  geom_text(aes(size=abs(coef)), hjust = 0, nudge_x = 0.002) + 
  geom_hline(yintercept=pthreshold,lty=2) +
  scale_size(range = c(1,6), guide = guide_legend(title=" ")) + 
  #theme_bw() +
  xlab("Coefficient MetS") + ylab("log10 P-value")

#metS groups
volc <- data.frame(var=gsub("_","-",names(icc.mets)),icc=icc.mets,coef=coef.mets1[,2],p=mod.mets1[,2]) %>% 
  mutate(plog=-log10(p), icc4=cut(icc, breaks=c(0,0.25,0.5,0.75,1)))
...
ggplot(volc, aes(y=plog, x=coef, label=var, color=icc4)) + 
  ...
  scale_size(range = c(1,6), guide = guide_legend(title=" ")) + 
  #theme_bw() +
  xlab("Coefficient MetS (groups)") + ylab("log10 P-value")

#BMI
volc <- data.frame(var=gsub("_","-",names(icc.mets)),icc=icc.mets,coef=coef.mets1[,3],p=mod.mets1[,3]) %>% 
  mutate(plog=-log10(p), icc4=cut(icc, breaks=c(0,0.25,0.5,0.75,1)))
...
#All variables
volc <- data.frame(var=gsub("_","-",names(icc.mets)),icc=icc.mets,metsc=coef.mets1[,1],metsp=mod.mets1[,1],
                   bmic=coef.mets1[,3],bmip=mod.mets1[,3],
                   trigc=coef.mets1[,4],trigp=mod.mets1[,4],
                   bpc=coef.mets1[,5],bpp=mod.mets1[,5],
                   glucc=coef.mets1[,6],glucp=mod.mets1[,6],
                   cholc=coef.mets1[,7],colp=mod.mets1[,7]) %>% 
         mutate(icc4=cut(icc, breaks=c(0,0.25,0.5,0.75,1)))

volc <- melt(volc, id=c("var","icc","icc4","metsc","bmic","trigc","bpc","glucc","cholc"))
volc <- melt(volc, id=c("var","icc","icc4","variable","value")) 
names(volc) <- c("var","icc","icc4","variable.p","value.p","variable.c","value.c")
# bonferroni threhsold
pthreshold = -log10(0.05/length(icc.mets))

ggplot(volc, aes(y=-log10(value.p), x=value.c, label=var, color=icc4)) + 
  geom_point(aes(size=abs(value.c))) + 
  geom_text(aes(size=abs(value.c)), hjust = 0, nudge_x = 0.002) + 
  geom_hline(yintercept=pthreshold,lty=2) +
  scale_size(range = c(1,6), guide = guide_legend(title=" ")) + 
  #theme_bw() +
  xlab("Coefficient") + ylab("log10 P-value") + facet_wrap(~variable.p)
...
dat.rep <- dat.rep %>% mutate(mets.diff=metsz.y-metsz.x, 
                              ...
                              sbt.diff=sbt.y-sbt.x, dbt.diff=dbt.y-dbt.x,
...
# Model fitting 
# Matrix to store estimate in:
mod1.d <- matrix(nrow=ncol(dat),ncol=length(cov))
coef1.d <- sd1.d <- matrix(nrow=ncol(dat),ncol=length(cov)+2)
...
for(i in var1){
  ...
  # MODELs lifestyle
    ...
    sd1[j,] <- s$tTable[-1,2]
 # MODELs MetS
    g1 <- lm(var~metsz.diff+casecontrol+age.x+gender.x+phys.diff+smoke.x,
    ...
    # P-values
    mod.mets1[j,1] <- anova(g1)$`p-value`[2]
    mod.mets1[j,2:6] <- anova(g3)$`p-value`[2:6]
    
    # Coefficients
    coef.mets1[j,1] <- summary(g1)$tTable[2,1]
    coef.mets1[j,2:6] <- summary(g3)$tTable[2:6,1]

    # sd
    sd.mets1[j,1] <- summary(g1)$tTable[2,2]
    sd.mets1[j,2:6] <- summary(g3)$tTable[2:6,2]
...
Circos correlations - signiricant in metS:

```{r circos corelations significants METS,echo=F}
#Data fix 
#significants in metS
mets.bh1 <- apply(mod.mets1,2,p.adjust,method="bonferroni")
mets.bh2 <- data.frame(var=rownames(mets.bh1),ifelse(mets.bh1<=0.05,1,0)) 
mets.sign <- mutate(mets.bh2,sum.sign=rowSums(mets.bh2[,2:7])) %>% filter(sum.sign>0) 
mets.sign <- mets.sign[order(mets.sign$var),]
...
choose <- as.character(mets.sign$var)
rdata1 <- res.proteins %>% dplyr::select(one_of(choose))
names(rdata1) <- gsub('_', '-', names(rdata1))
...
mets.cpg <- read.csv("~/Documents/DATA-filer/Sophia early detection/methylation/MetS CpGs.csv", stringsAsFactors=FALSE)

cpg1 <- mets.cpg$IlmnID
#merge with data frame:
beta1 <- dplyr::select(beta1, id, Barcodesection, one_of(cpg1))
sum(beta1$id %in% dat1$id)
dat1.m <- merge(dat1, beta1, by="id")
...
# Data fix  8*8 inch pic
dat11 <- dplyr::select(dat1.m, provnr, casecontrol, person, one_of(cpg1))  %>% 
          mutate(case=factor(casecontrol,labels=c("control","case")),
                 case.provnr=interaction(case,provnr),cpnum=as.numeric(case.provnr)) %>% 
          unite(person_case_provnr,case.provnr, person, sep=" ",remove=F)

# remove NAs
con <- dplyr::select(dat11, one_of(cpg1),person_case_provnr,cpnum) %>% arrange(cpnum) %>% dplyr::select(-cpnum)
...
# Data fix 8*8 inch pic
# Remove many NAs
na1 <- apply(dat1.m[,cpg1],2,function(x) sum(is.na(x))/length(x))
plot(na1);abline(h=0.5)
na1[na1>=0.15]
manyna1 <- names(na1[na1>=0.15])
...
#
# Data - provnr 
corre <-  dplyr::select(dat1.m,provnr, gender, age4, bmi2, stg2, ht2, gluc2, mets2, one_of(var)) %>%
        dplyr::select(-one_of(c(manyna1))) %>% 
        mutate(bmi2=coalesce(as.integer(bmi2), 9999L), stg2=coalesce(as.integer(stg2), 9999L),
               ht2=coalesce(as.integer(ht2), 9999L), mets2=coalesce(as.integer(mets2), 9999L)) 
