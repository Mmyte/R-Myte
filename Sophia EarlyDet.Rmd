...
ggplot(datn,aes(x=value,fill=as.factor(provnr)))+geom_histogram()+theme_bw()+facet_grid(variable ~ .)
...
dat <- select(dat2r,starts_with("1"))

#calculate models
models1 <- lapply(dat, function(q){
   glm(q ~ dat2r$casecontrol+dat2r$gender+dat2r$langd+dat2r$vikt)
    })
